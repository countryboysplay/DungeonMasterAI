...
public sealed class DmToolRouter(GameEngine engine, DiceService dice, RulesSearchService rules)
{
    ...
    [
        ...
        Tool("cast_projectile_spell", "Cast a deterministic multi-projectile spell such as Magic Missile or Scorching Ray. Auto-hit projectiles resolve automatically. Player-character projectile attack spells pause for each player attack and damage roll in firing order, including intervening Concentration saves. target_ids is either one target for every projectile or one target id per projectile in firing order.", Props(("character_id","string",true),("spell_id","string",true),("target_ids","array",true),("slot_level","integer",false),("encounter_id","string",false))),
