...
public sealed partial class DmToolRouter(GameEngine engine, DiceService dice, RulesSearchService rules)
{
    ...
    [
        ...
        Tool("damage_character", "Apply typed damage, including Temporary HP, Resistance, Vulnerability, Immunity, and 0 HP rules. NPC Concentration saves resolve automatically; player-character Concentration saves create a required player d20 roll and stop progression until resolved.", Props(("character_id","string",true),("amount","integer",true),("damage_type","string",false),("critical_hit","boolean",false))),
        ...
        Tool("cast_spell", "Cast one prepared spell through the deterministic spell engine. Player-character spell attacks create required player attack and damage rolls. A player character targeted by a single-target saving-throw spell receives a required player d20 save unless the rules make that save automatically fail. When a player character casts a damaging saving-throw spell, the target save is resolved first and the caster then receives a required player damage roll whenever damage applies. NPC spell attacks, NPC saving throws, and NPC-caster damage dice still resolve automatically. The engine validates spell slots, components, turn limits, healing, upcasting, Rituals, and Concentration.", Props(("character_id","string",true),("spell_id","string",true),("target_id","string",false),("slot_level","integer",false),("as_ritual","boolean",false),("encounter_id","string",false))),
        Tool("cast_projectile_spell", "Cast a deterministic multi-projectile spell such as Magic Missile or Scorching Ray. NPC auto-hit projectiles resolve automatically. Player-character auto-hit projectiles pause for each player damage roll. Player-character projectile attack spells pause for each player attack and damage roll in firing order. Both pipelines pause for intervening Concentration saves. target_ids is either one target for every projectile or one target id per projectile in firing order.", Props(("character_id","string",true),("spell_id","string",true),("target_ids","array",true),("slot_level","integer",false),("encounter_id","string",false))),
        ...
        Tool("take_hide", "Take the 2024 Hide action. The engine validates concealment and line of sight. NPC Stealth resolves immediately; a player character creates a required Dexterity (Stealth) d20 roll and pauses until the Game Table supplies it.", Props(("encounter_id","string",true),("combatant_id","string",true))),
        Tool("search_hidden", "Use the Search action with Wisdom (Perception) against a selected hidden combatant's recorded Hide total. NPC Search resolves immediately; a player character creates a required d20 roll and pauses until the Game Table supplies it.", Props(("encounter_id","string",true),("searcher_combatant_id","string",true),("target_combatant_id","string",true))),
        ...
        Tool("trigger_readied_attack", "Confirm that a readied attack trigger occurred. NPC reactions resolve automatically. A player character receives an explicit Reaction choice; accepting it creates required player-owned attack and damage rolls.", Props(("encounter_id","string",true),("combatant_id","string",true))),
        Tool("trigger_readied_move", "Confirm that a readied-movement trigger occurred and propose a legal destination. NPC movement resolves automatically. A player character receives an explicit Reaction choice before any movement is committed; Opportunity Attacks and battlefield hazards still resolve through the authoritative engine.", Props(("encounter_id","string",true),("combatant_id","string",true),("grid_x","integer",true),("grid_y","integer",true))),
        Tool("trigger_readied_spell", "Confirm that a readied spell trigger occurred and identify its release target when needed. NPC reactions release automatically. A player character receives an explicit Reaction choice, and any player-owned attack, save, or damage dice are requested before resolution continues.", Props(("encounter_id","string",true),("combatant_id","string",true),("target_combatant_id","string",false))),
        ...
        Tool("first_aid", "Use the Help action to administer first aid with a DC 10 Wisdom (Medicine) check. NPC checks resolve immediately; a player character creates a required d20 roll before stabilization or awakening can occur.", Props(("encounter_id","string",true),("helper_combatant_id","string",true),("target_combatant_id","string",true))),
        Tool("take_search", "Take the Search action using Insight, Medicine, Perception, or Survival against a DM-selected DC. NPC checks resolve immediately; a player character creates a required player d20 roll and pauses until the Game Table supplies it.", Props(("encounter_id","string",true),("combatant_id","string",true),("skill","string",true),("dc","integer",true))),
        Tool("take_study", "Take the Study action using Arcana, History, Investigation, Nature, or Religion against a DM-selected DC. NPC checks resolve immediately; a player character creates a required player d20 roll and pauses until the Game Table supplies it.", Props(("encounter_id","string",true),("combatant_id","string",true),("skill","string",true),("dc","integer",true))),
        Tool("take_influence", "Take the Influence action using an appropriate Charisma check, or Wisdom (Animal Handling), against a DM-selected DC. NPC checks resolve immediately; a player character creates a required player d20 roll and pauses until the Game Table supplies it.", Props(("encounter_id","string",true),("combatant_id","string",true),("skill","string",true),("dc","integer",true))),
    ...
    public DmToolResult Execute(CampaignState campaign, string toolName, string argumentsJson)
    {
        try
        {
            ...
            object? result = toolName switch
            {
                ...
                "take_hide" => ResolveHideTool(engine, dice, campaign, a),
                "search_hidden" => ResolveHiddenSearchTool(engine, dice, campaign, a),
                ...
                "trigger_readied_attack" => ResolveReadiedAttackTriggerTool(engine, dice, campaign, a),
                "trigger_readied_move" => ResolveReadiedMoveTriggerTool(engine, campaign, a),
                "trigger_readied_spell" => ResolveReadiedSpellTriggerTool(engine, dice, campaign, a),
                ...
                "first_aid" => ResolveFirstAidTool(engine, dice, campaign, a),
                "take_search" => ResolveSearchActionTool(engine, dice, campaign, a),
                "take_study" => ResolveStudyActionTool(engine, dice, campaign, a),
                "take_influence" => ResolveInfluenceActionTool(engine, dice, campaign, a),
                "unarmed_grapple" => ResolveUnarmedGrappleTool(engine, dice, campaign, a),
                "unarmed_shove" => ResolveUnarmedShoveTool(engine, dice, campaign, a),
                "escape_grapple" => ResolveEscapeGrappleTool(engine, dice, campaign, a),
