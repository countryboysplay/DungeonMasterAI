...
public sealed partial class GameEngine
{
    internal void SyncOpportunityAttackPlayerDecision(CampaignState campaign, EncounterState encounter)
    {
        ...
        foreach (var window in pendingMove.OpportunityAttacks.Where(x => !x.Resolved))
        {
            ...
            if (reactorCombatant is null) continue;
            ...
            if (reactor is null
                || !reactor.CharacterType.Equals("pc", StringComparison.OrdinalIgnoreCase)
                || !reactorCombatant.ReactionAvailable
                || !CanTakeReaction(reactor))
                continue;
            ...
            {
                // There is no legal melee attack to offer. Treat the reaction window as declined
                // so the deterministic move cannot deadlock on an impossible player choice.
